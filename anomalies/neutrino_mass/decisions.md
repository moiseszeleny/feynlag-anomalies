...
One dated line per ladder peldaño (kickoff spec §5), tagged `[feynlag-verified: test]` (backed by
...
- **2026-09-22 — Peldaño 0** (dimensional estimate): `m_nu ~ y^2 v^2 / M` implemented in
  `solutions/peldano_0.py::m_nu_estimate`. `[physics judgment]` — the standard seesaw-scale
  dimensional estimate, not itself a feynlag computation.
- **2026-09-22 — Peldaño 1** (SM structural test): `feynlag.suggest.suggest_yukawa` on minimal SM
...
- **2026-09-22 — Peldaño 2** (EFT operator): same call at `max_dim=5` surfaces exactly one
...
- **2026-09-22 — Peldaño 3** (UV completion, choice of minimal model): type-I seesaw chosen over
...
- **2026-09-22 — Peldaño 4** (predict before running): read off `bundle.extra["MN1"]` (light),
  ...
  **Caveat, `[physics judgment]`**: `seesaw_type1` is single-generation, so "how many light
  ...
  measured) is posed as the reader's own derivation feeding directly into peldaño 5, not literally
  instantiated by this model.
- **2026-09-22 — Peldaño 5** (breaking it on purpose / minimal extension request): two
...
- **2026-09-22 — Peldaño 6** (stage-1 χ² against oscillation data): **SKIPPED — blocked.** A
  ...
  this session; peldaño 6 unblocks once `model_requests/seesaw_type1_nN.md` is approved and built.
